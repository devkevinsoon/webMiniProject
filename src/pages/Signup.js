...
const Signup = (props) => {
    ...
    if (emailCheck(user_name)) {
      try {
        const checkUserName = await axios.post(
          "http://54.180.96.119/api/userName",
          {
            username: user_name,
          }
        );
  ...
  const checkNickNameDup = async () => {
    if (nickname.length >= 2) {
      try {
        const checkUserNickName = await axios.post(
          "http://54.180.96.119/api/nickName",
          {
            nickname: nickname,
          }
        );
        ...
        <Grid height="60%" is_flex column>
          <Grid is_flex column position="relative">
            ...
            {user_name && !emailCheck(user_name) && (
              <Span color="red">이메일 형식이 아닙니다.</Span>
            )}
            {user_name && emailCheck(user_name) && (
              <Span color="blue">올바른 이메일 형식입니다.</Span>
            )}
          </Grid>
          <Grid is_flex column position="relative">
            ...
            {nickname && nickname.length < 2 && (
              <Span color="red">닉네임은 최소 2자 이상입니다.</Span>
            )}
            {nickname && nickname.length >= 2 && (
              <Span color="blue">올바른 닉네임 형식입니다.</Span>
            )}
          </Grid>
          <Grid is_flex column position="relative">
            ...
            {pwd && !pwdCheck(pwd) && (
              <Span color="red">
                패스워드는 숫자와 특수문자를 포함한 최소 8자 이상입니다.
              </Span>
            )}
            {pwd && pwdCheck(pwd) && (
              <Span color="blue">올바른 패스워드 형식입니다.</Span>
            )}
          </Grid>
          <Grid is_flex column position="relative">
            <Grid margin="0px 0px 30px 0px" is_flex>
